...
class Front_Conf_Map_Form_LC_HC_MVC extends _HC_Form
{
	...
	public function render_advanced()
	{
		...
		$links
			->add(
				$this->make('/html/view/link')
					->to('/front.conf/map/mode', array('to' => 'basic'))
		...
		if( $this_value_modified != $this_value_default ){
			$links
				->add(
					$this->make('/html/view/link')
						->to('/front.conf/map/mode', array('to' => 'reset'))
	...
	public function render(){
		...
		$link_to_advanced = $this->app->make('/html/view/link')
			->to('/front.conf/map/mode', array('to' => 'advanced'))
