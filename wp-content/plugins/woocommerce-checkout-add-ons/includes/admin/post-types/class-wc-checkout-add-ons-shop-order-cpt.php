...
class WC_Checkout_Add_Ons_Shop_Order_CPT {
	...
	/** Separator used between add-on name and selected/entered value in order review area */
		...
		foreach ( wc_checkout_add_ons()->get_add_ons() as $add_on ) {
			...
			if ( $add_on->is_filterable() ) {
				...
				if ( $add_on->has_options() ) {
					...
					?>
					<select name="<?php echo esc_attr( $add_on->get_key() ); ?>" id="<?php echo esc_attr( $add_on->get_key() ); ?>" class="wc-enhanced-select" data-placeholder="<?php echo esc_attr( __( "Show all ", 'woocommerce-checkout-add-ons' ) . $add_on->name ); ?>" data-allow_clear="true" style="min-width:200px;">
						<option value=""></option>
						<?php
						foreach ( $add_on->get_options() as $option ) :
							...
							$value = sanitize_title( $option['label'], '', 'wc_checkout_add_ons_sanitize' );

							echo '<option value="' . esc_attr( $value ) . '" ' . ( isset( $_GET[ $add_on->get_key() ] ) ? selected( $value, $_GET[ $add_on->get_key() ] ) : '' ) . '>' . esc_html__( $option['label'], 'woocommerce-checkout-add-ons' ) . '</option>';
					...
					<input type="hidden" class="sv-wc-enhanced-search" name="<?php echo esc_attr( $add_on->get_key() ); ?>" style="min-width:200px;"
						data-placeholder="<?php echo __( "Show all ", 'woocommerce-checkout-add-ons' ) . $add_on->label; ?>"
