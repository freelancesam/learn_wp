...
class Location_model extends MY_Model
{
	...
	const ORDER_COLLATE = 'utf8_swedish_ci';
	...
	function make_address( $e, $for_display = FALSE, $append_search = TRUE )
	{
		if( $for_display ){
			...
			$CI =& ci_get_instance();
			$template = $CI->app_conf->get( 'address_format' );
...
*/
	...
	function get_all_by_search( $search2 = array() )
	{
		...
		if( self::ORDER_COLLATE ){
			$this->db->order_by('name COLLATE ' . self::ORDER_COLLATE, 'ASC');
		}
		else {
	...
	/* types */
		...
		$sort_output = $this->app_conf->get('sort_output');
		if( $sort_output && ($sort_output != 'distance') && (substr($sort_output, 0, strlen('misc')) == 'misc') ){
			$this->db->order_by( "$sort_output IS NULL", 'ASC' );
			$this->db->order_by( $sort_output, 'ASC' );
		}
