...
<div class="span1">
	<?php
	$perpage_view = isset($perpage_options[$perpage]) ? $perpage_options[$perpage] : $perpage;
	?>
	<div class="btn-group">
		<a class="btn dropdown-toggle" data-toggle="dropdown" href="#">
			<?php echo $perpage_view; ?>
			<span class="caret"></span>
		</a>
		<ul class="dropdown-menu" role="menu" aria-labelledby="dropdownMenu">
			<?php foreach( $perpage_options as $ppo => $ppl ) : ?>
				<?php if( $ppo == $perpage ){ continue; } ?>
				<li><?php echo ci_anchor( array($this->conf['path'], 'perpage', $ppo), $ppl ); ?></li>
			<?php endforeach; ?>
		</ul>
	</div>
</div>
...
$span = isset($span_per_row[$per_row]) ? $span_per_row[$per_row] : 'span4';
?>
...
{
	// $link_title = htmlentities($e['name']);
	$link_title = $e['name'];
}
?>
