...
<div class="page-header">
	<div class="row-fluid">
		<div class="span8">
			<h2 style="margin: 0 0; padding: 0 0;"><?php echo lang('location_edit');?></h2>
		</div>
...
<div class="row-fluid">
	...
	<div class="span7">
		...
		<?php echo form_open('', array('class' => 'form-condensed')); ?>
		...
		<?php foreach( $fields as $f ) : ?>
			...
			<div class="control-group <?php echo $class; ?>">
				...
				<label class="control-label muted" style="font-size: .9em;" for="<?php echo $f['name']; ?>">
				...
				<div class="controls">  
					...
					switch( $type )
					{
						...
						case 'textarea':
							...
							break;
						default:
							echo form_input( $f, set_value($f['name'], $object[$f['name']]), 'style="width: 95%;"');
							break;
					}
					?>
					<?php if( $error ) : ?>
	...
	<div class="span5" style="margin-bottom: 0.5em;">

		<ul class="list-unstyled list-margin-v" style="margin-bottom: 0.5em;">
			<li>
				<a data-toggle="collapse" href="#hc-collapse-manual-entry">
					Manual coordinates entry
				</a>
			</li>
			<li id="hc-collapse-manual-entry" class="collapse">
<?php
$coord_fields = array(
	array(
		'name'		=> 'latitude',
...
?>
				<?php echo form_open('admin/locations/coord/' . $object['id'], array('class' => 'form-condensed', 'id' => 'lpr-manual-coord-form')); ?>
				<?php foreach( $coord_fields as $f ) : ?>
					<div class="control-group <?php echo $class; ?>">
						<label class="control-label muted" style="font-size: .9em;" for="<?php echo $f['name']; ?>">
							<?php echo $f['title']; ?><?php if( $required ){echo ' *';}; ?>
						</label>
				...
				<?php echo form_button( array('type' => 'submit', 'name' => 'submit', 'class' => 'btn btn-primary'), lang('common_save')); ?>
				<?php echo ci_anchor( array($this->conf['path'], 'resetcoord', $object['id']), 'Reset', 'class="btn btn-warning hc-confirm"' ); ?>
				<?php echo form_close();?>
			</li>
		</ul>
					...
					function( results, status )
					{
						switch( status )
						{
							...
							// save coordinates
								...
								.done( function()
								{
									jQuery("#lpr-manual-coord-form").find('[name=latitude]').val(loc.lat());
									jQuery("#lpr-manual-coord-form").find('[name=longitude]').val(loc.lng());
