...
class Locatoraid_Base
{
	...
	// database
		$db_host = DB_HOST;
		if( strpos($db_host, ':') ){
			list( $db_host, $db_host_port ) = explode(':', $db_host);
...
EOT;
	...
	{
		if( isset($_POST['subscribe_data_array']) ){
			return;
		}

	// gravity forms on the same page
...
//		wp_enqueue_script( 'lctrScript' . $ii++, plugins_url('assets/bootstrap/js/bootstrap.min.js', $this->full_path) );
		wp_enqueue_script( 'lctrScript1', plugins_url('assets/js/lpr.js', $this->full_path), array('jquery') );
		wp_enqueue_script( 'lctrScript_front', plugins_url('assets/js/lpr-front.js', $this->full_path), array('jquery') );
	}
	...
	function admin_print_scripts()
	{
		$ii = 1;
		wp_enqueue_script( 'jquery' );
		wp_enqueue_script( 'lctrScript' . $ii++, plugins_url('assets/bootstrap/js/bootstrap.min.js', $this->full_path), array('jquery') );
		wp_enqueue_script( 'lctrScript' . $ii++, plugins_url('assets/js/lpr.js', $this->full_path), array('jquery') );
	}
}
}
?>