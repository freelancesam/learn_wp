...
//                console.log(presponse.currentTarget, presponse.currentTarget.responseText, file.name);
            ...
            if(args.cthis){
                //console.info(args, ftype);

                if(args.cthis.hasClass('only-image')){
                    if(ftype!='image/png' && ftype!='image/jpeg' && ftype!='image/gif'){
                        if (args._feedback) {
                            //console.log(args.cthis);
                            args._feedback.html('<div class="error">only image allowed</div>');
...
//            console.info(args._progress);
            ...
            if (args != undefined && args.uploadlist != undefined) {
                for (i = 0; i < $(args.uploadlist).children().length; i++) {
                    var _cache = $(args.uploadlist).children().eq(i);
...
//            console.log($(this));
            ...
            if(cthis.hasClass('drag-drop')==false){
                ...
                aux+='<input type="button" value="Upload" class="btn_upl';
