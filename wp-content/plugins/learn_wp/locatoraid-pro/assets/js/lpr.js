...
{
    console.log(loc, search2, address, allow_empty, within)
...
//			alert( "3 = " + csrfToken );
                var priority = jQuery('#lpr-search-form').find('[name=priority]').val();
                var thisData = {
...
// get matching locations
    lpr_offset = 0;
    console.log('lpr_front_pull_results 2');
...
{
    console.log('lpr_front_get_results 1');
    allow_empty = (typeof allow_empty === "undefined") ? false : allow_empty;
    if (address.length < 1)
    {
        console.log('lpr_front_pull_results 1');
                    ...
                    function (results, status)
                    {
                        switch (status)
                        {
                            ...
                            case google.maps.GeocoderStatus.OK :
                                console.log('lpr_front_get_results 2');
                                ...
                                switch (status)
                                {
                                    case google.maps.GeocoderStatus.OVER_QUERY_LIMIT :
                                        if (lpr_next_location.attempts < 3)
                                        {
                                            ...
                                            setTimeout(function () {
                                                lpr_next_location(url_prefix, loc_id, map)
                                            }, 2000);
                                        } else
                                        {
                                            // daily limit reached
...
function lpr_search_form_submit(search_form)
{
    ...
    var priority = search_form.find('[name=priority]').val();
    if (country)
    {
        if (address)
