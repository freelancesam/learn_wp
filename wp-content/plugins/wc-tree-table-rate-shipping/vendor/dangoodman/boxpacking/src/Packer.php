...
class Packer
{
    ...
    private static function place($box, $items)
    {
        ...
        $projections = Utils::generateProjections($items);
        $sideProjections = Utils::generateProjections($sideItems);

        unset($items, $sideItems);


        $sideSkyline = new Skyline($sideBox);

        Utils::fillSkyline($sideProjections, $sideSkyline, function ($bestItemIndex, $bestSideProjection) use ($box, &$sideProjections, &$projections) {

            $frontSkyline = new Skyline(array($box[0], $bestSideProjection[0], $bestSideProjection[1]));
            ...
            Utils::fillSkyline($projections, $frontSkyline, function ($bestItemIndex) use (&$sideProjections, &$projections) {
                unset($projections[$bestItemIndex], $sideProjections[$bestItemIndex]);
            });
        });
