<?php
if (!function_exists('tn_block10')) {
    function tn_block10($post)
    {
        ...
        if (function_exists("has_post_thumbnail") && has_post_thumbnail($post->ID)) {
            ...
            $str .= get_the_post_thumbnail($post->ID, 'module_5_thumb') . '</a>';
        } else {
            $str .= '<div class="no-thumb-wrap"><a class="no-thumb"  href="' . $href . '" title="' . $title_attribute . '" rel="bookmark"></a></div>';
        };
        $str .= '<div class="block4-cate-tag">' . tn_cate_tag($post) . '</div>';
        $str .= '</div>';
