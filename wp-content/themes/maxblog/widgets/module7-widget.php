<?php
add_action('widgets_init', 'tn_register_module7_widget');

function tn_register_module7_widget()
{
    register_widget('tn_module7_widget');
}

class tn_module7_widget extends WP_Widget
{

    function tn_module7_widget()
    {
        $widget_ops = array('classname' => 'module7-widget', 'description' => __('[Content Widget] This widget can place in PRIMARY CONTENT or FULL WIDTH. please, Enable 2 Columns layout if you place it into FULL WIDTH','tn'));
        $this->WP_Widget('module7-widget', __('TN . POSTS BOX 7', 'tn'), $widget_ops);
    }
    ...
    function widget($args, $instance)
    {
        ...
        $options['col_2'] = ($instance['col_2']) ? $instance['col_2'] : '';
        ...
        if($title == 'default'){
            ...
            } else $title = esc_attr__('POSTS BOX 7','tn');
        }
        ...
        if ($query_data) :?>
            <div id="<?php echo $id; ?>">
                <?php  echo tn_module7($query_data->posts ,$options); ?>
            </div><!-- #row -->
        <?php endif;
    ...
    function update($new_instance, $old_instance)
    {
        ...
        $instance['col_2'] = strip_tags($new_instance['col_2']);
    ...
    {
        $defaults = array('title' => 'default', 'sort_order' => 'date_post', 'pagination' => 'next_prev', 'posts_per_page' => 6, 'cate' => '', 'cates' => '', 'child_cate' => '', 'num_child_cate' => '', 'tags' => '', 'readmore' => '', 'excerpt' => 18, 'col_2' => '');
        ...
        <p>
            <label for="<?php echo $this->get_field_id( 'col_2' ); ?>"><?php _e('2 Columns Layout:','tn') ?></label>
            <input class="widefat" type="checkbox" id="<?php echo $this->get_field_id( 'col_2' ); ?>" name="<?php echo $this->get_field_name( 'col_2' ); ?>" value="checked" <?php if( !empty( $instance['col_2'] ) ) echo 'checked="checked"'; ?>  />
        </p>
    <?php
    }
}
