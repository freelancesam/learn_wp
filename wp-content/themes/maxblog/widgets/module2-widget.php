<?php
add_action('widgets_init', 'tn_register_module2_widget');

function tn_register_module2_widget()
{
    register_widget('tn_module2_widget');
}

class tn_module2_widget extends WP_Widget
{

    function tn_module2_widget()
    {
        $widget_ops = array('classname' => 'module2-widget', 'description' =>  __('[Primary Content Widget] This widget can place in PRIMARY CONTENT','tn'));
        $this->WP_Widget('module2-widget', __('TN . POSTS BOX 2', 'tn'), $widget_ops);
    }
    ...
    function widget($args, $instance)
    {
        ...
        $options['style'] = ($instance['style']) ? $instance['style'] : '';
        ...
        if($title == 'default') {
            ...
            } else $title = esc_attr__('POSTS BOX 2','tn');
        }
	   ...
	   <?php if ($query_data) :?>
		    <div id="<?php echo $id; ?>" class="row">
			    <?php  echo tn_module2($query_data->posts ,$options); ?>
		    </div><!-- #row -->
	    <?php endif;
    ...
    {
        $defaults = array('title' => 'default', 'video_thumb' => '', 'style'=>'1', 'sort_order' => 'date_post', 'pagination' => 'next_prev', 'cate' => '', 'cates' => '', 'child_cate' => '', 'num_child_cate' => '', 'tags' => '', 'excerpt' => 18, 'readmore' => '');
