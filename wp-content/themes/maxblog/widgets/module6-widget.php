<?php
add_action('widgets_init', 'tn_register_module6_widget');

function tn_register_module6_widget()
{
    register_widget('tn_module6_widget');
}

class tn_module6_widget extends WP_Widget
{

    function tn_module6_widget()
    {
        $widget_ops = array('classname' => 'module6-widget', 'description' => __('[Content Widget] This widget can place in PRIMARY CONTENT or FULL WIDTH. please, Enable 3 Columns layout if you place it into FULL WIDTH','tn'));
        $this->WP_Widget('module6-widget', __('TN . POSTS BOX 6', 'tn'), $widget_ops);
    }
    ...
    function widget($args, $instance)
    {
        ...
        $options['thumb'] = ($instance['thumb']) ? $instance['thumb'] : 'medium';
        ...
        if($title == 'default') {
            ...
            } else $title = esc_attr__('POSTS BOX 6','tn');
        }
        ...
        <?php  if ($query_data) :?>
            <div id="<?php echo $id; ?>">
                <?php  echo tn_module6($query_data->posts ,$options); ?>
            </div><!-- #row -->
        <?php endif;
    ...
    {
        $defaults = array('title' => 'default', 'sort_order' => 'date_post', 'pagination' => 'next_prev', 'child_cate' => '', 'num_child_cate' => '', 'posts_per_page' => 6, 'cate' => '', 'cates' => '', 'tags' => '', 'col_3'=>'','thumb'=>'medium');
        ...
        <p>
            <label for="<?php echo $this->get_field_id( 'thumb' ); ?>"><?php _e('Thumb Size: ', 'tn'); ?></label>
            <select class="widefat" id="<?php echo $this->get_field_id( 'thumb' ); ?>" name="<?php echo $this->get_field_name( 'thumb' ); ?>" >
                <option value="medium" <?php if( !empty($instance['thumb']) && $instance['thumb'] == 'medium' ) echo "selected=\"selected\""; else echo ""; ?>><?php _e('Medium', 'tn'); ?></option>
                <option value="big" <?php if( !empty($instance['thumb']) && $instance['thumb'] == 'big' ) echo "selected=\"selected\""; else echo ""; ?>><?php _e('Big', 'tn'); ?></option>
            </select>
        </p>
        <p>
        ...
        <p>
            <label for="<?php echo $this->get_field_id( 'sort_order' ); ?>"><trong><?php _e('Order By:', 'tn'); ?></trong></label>
